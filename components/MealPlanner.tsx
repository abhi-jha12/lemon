...
import { Plus, Clock, Flame, ChevronDown, ChevronUp, X, Check, Loader2 } from 'lucide-react'
...
type CustomForm = { name: string; calories: string; protein: string; carbs: string; fat: string; emoji: string }
const EMPTY_FORM: CustomForm = { name: '', calories: '', protein: '', carbs: '', fat: '', emoji: '🍽️' }
...
export default function MealPlanner() {
  ...
  const [expanded, setExpanded]   = useState<Slot | null>('Breakfast')
  ...
  const mealsBySlot = SLOTS.reduce((acc, s) => {
    acc[s] = meals.filter((m: MealLog) => m.meal_slot === s)
    return acc
  }, {} as Record<Slot, MealLog[]>)

  // Filter templates relevant to the slot (or General)
  const getTemplatesForSlot = (slot: Slot): MealTemplate[] => {
    if (!allTemplates) return []
    return allTemplates.filter(t => t.category === slot || t.category === 'General')
  }

  const handleAddTemplate = async (slot: Slot, tpl: MealTemplate) => {
    setSaving(true)
    try {
      await addMeal({ meal_slot: slot, name: tpl.name, emoji: tpl.emoji, calories: tpl.calories, protein: tpl.protein, carbs: tpl.carbs, fat: tpl.fat })
  ...
  const handleAddCustom = async (slot: Slot) => {
    ...
    setSaving(true)
    try {
      await addMeal({ meal_slot: slot, name: form.name, emoji: form.emoji, calories: +form.calories || 0, protein: +form.protein || 0, carbs: +form.carbs || 0, fat: +form.fat || 0 })
      showToast(`Added ${form.name}`)
      setForm(EMPTY_FORM); setShowCustom(null)
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="font-display text-2xl font-bold text-lemon-300">
              {loading ? '…' : totals.calories} <span className="text-sm text-lemon-100/40">kcal</span>
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs text-lemon-100/50 mb-1">Protein</p>
            <p className="font-display text-xl font-bold text-forest-400">{totals.protein}g</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-lemon-100/50 mb-1">Remaining</p>
            <p className="font-display text-xl font-bold text-lemon-200">{Math.max(CALORIE_GOAL - totals.calories, 0)}</p>
          </div>
        </div>
        <div className="progress-bar h-2 mt-3">
          <div className="progress-fill" style={{ width: `${Math.min((totals.calories / CALORIE_GOAL) * 100, 100)}%` }} />
        </div>
      </div>
        ...
        <div key={slot} className="card overflow-hidden">
          <button className="w-full flex items-center justify-between p-4"
            ...
            <div className="flex items-center gap-3">
              <div className={`px-2.5 py-1 rounded-lg border text-xs font-semibold ${SLOT_COLORS[slot]}`}>{slot}</div>
              <span className="text-xs text-lemon-100/40">
                {mealsBySlot[slot].reduce((s, m) => s + m.calories, 0)} kcal
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-lemon-100/30">{mealsBySlot[slot].length} items</span>
          ...
          {expanded === slot && (
            <div className="px-4 pb-4 space-y-2 animate-fade-in">
              {loading && <p className="text-xs text-lemon-100/30 text-center py-2">Loading…</p>}
              ...
              {mealsBySlot[slot].map(meal => (
                <div key={meal.id} className="flex items-center gap-3 bg-bark-800/60 rounded-xl p-3">
                  ...
                  <button onClick={() => deleteMeal(meal.id)} className="text-lemon-100/20 hover:text-red-400 transition-colors flex-shrink-0">
                    <X size={14} />
                  </button>
                </div>
              ))}
              ...
              <div className="flex gap-2 mt-2">
                <button onClick={() => { setShowSuggest(showSuggest === slot ? null : slot); setShowCustom(null) }}
                ...
                <button onClick={() => { setShowCustom(showCustom === slot ? null : slot); setShowSuggest(null) }}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-dashed border-lemon-500/30 text-xs text-lemon-400/70 hover:border-lemon-400/60 hover:text-lemon-400 transition-all">
                  <Plus size={12} /> Custom
                </button>
              </div>

              {/* Template suggestions from DB */}
              {showSuggest === slot && (
                <div className="space-y-2 animate-fade-in">
                  <p className="text-xs text-lemon-100/40 px-1">
                    {tplLoading ? 'Loading meals…' : `${getTemplatesForSlot(slot).length} options for ${slot}:`}
                  </p>
                  {tplLoading && <div className="flex justify-center py-3"><Loader2 size={16} className="text-lemon-400/50 animate-spin" /></div>}
                  {getTemplatesForSlot(slot).map(tpl => (
                    <button key={tpl.id} onClick={() => handleAddTemplate(slot, tpl)} disabled={saving}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-lemon-100/40">{tpl.calories} kcal · P:{tpl.protein}g · C:{tpl.carbs}g · F:{tpl.fat}g</p>
                      </div>
                  ...
                  {!tplLoading && getTemplatesForSlot(slot).length === 0 && (
                    <p className="text-xs text-lemon-100/25 text-center py-2 italic">No templates yet — admin can add them</p>
                  )}
                </div>
              )}
                ...
                <div className="bg-bark-700/50 rounded-xl p-3 space-y-2 animate-fade-in">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[['calories','Kcal *'],['protein','Protein g'],['carbs','Carbs g'],['fat','Fat g']].map(([k, l]) => (
                      <input key={k} type="number" value={form[k as keyof CustomForm]}
                  ...
                  <button onClick={() => handleAddCustom(slot)} disabled={!form.name || !form.calories || saving}
                    ...
                    {saving ? <><Loader2 size={14} className="animate-spin" /> Saving…</> : 'Add Meal'}
                  </button>
                </div>
              )}
            </div>
